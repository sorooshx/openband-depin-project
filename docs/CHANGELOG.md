...
- **H4c gateway target pivoted to OpenWRT-first**. Userspace `wireguard-go` on macOS needs root to create `utun` devices, which would force an SMAppService privileged helper for a proper Mac gateway. OpenWRT has kernel WireGuard natively and no privilege issue — and it's the real production deployment target anyway (Starlink → OpenWRT router → phones). Mac stays on SOCKS5 for local dev. Collapses part of H4c into M1.
